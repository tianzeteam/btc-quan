...
class BinanceClient:
    def __init__(self):
        self.binance_client_usdt = None
        self.binance_client_coin = None

    def init_binance_from_app(self, app):
        self.binance_client_usdt = UsdtClient(api_key=app.config['BINANCE_API_KEY'],
                                              secret_key=app.config['BINANCE_API_SECRET'],
                                              url=app.config['BASE_REST_API_URL'])

        self.binance_client_coin = CoinClient(api_key=app.config['BINANCE_API_KEY'],
                                              secret_key=app.config['BINANCE_API_SECRET'],
                                              url=app.config['BASE_REST_API_URL'])
    ...
    def get_position_info(self, symbol):
        if symbol == 'BTCUSDT':
            ...
            for x in coin_list:
                ...
                if x_symbol == symbol:
                    return getattr(x, 'entryPrice'), getattr(x, 'positionAmt')
        return getattr(usdt_list[0], 'entryPrice'), getattr(usdt_list[0], 'positionAmt')
    ...
    def get_order(self, symbol, order_id):
        binance_client_api = self.binance_client_usdt if symbol == 'BTCUSDT' else self.binance_client_coin
        result = binance_client_api.get_order(symbol=symbol, orderId=order_id)
        avgPrice = getattr(result, 'avgPrice')
        status = getattr(result, 'status')
        return avgPrice, status
